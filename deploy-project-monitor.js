...
async function deployProject() {
    ...
    try {
        ...
        // === 第4步: 打开终端查看日志 ===
        console.log('\n📺 Step 4: Opening terminal to view logs...');
        await openTerminal();
...
async function openTerminal() {
    ...
    try {
        ...
        console.log('📺 Opening Pymakr terminal...');
        
        // 使用screen命令直接监控串口 - 更简单可靠
        console.log('📺 Device monitor will start in 3 seconds...');
        console.log('📺 To exit screen: Press Ctrl+A then K (kill) or Ctrl+A then D (detach)');
        console.log('📺 Please remember: Ctrl+A then K to exit');
        
        // 等待3秒让用户看清退出命令
        await new Promise(resolve => setTimeout(resolve, 3000));
        
        console.log('📺 Starting monitor now...');
        ...
        console.log('⚠️  Could not auto-open terminal:', error.message);
        console.log('📺 Please manually run: Ctrl+Shift+P → "Pymakr: Create Terminal"');
    }
}
